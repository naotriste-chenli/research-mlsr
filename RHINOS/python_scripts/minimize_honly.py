...
ff = ForceField('amber19-all.xml', 'implicit/gbn2.xml')
modeller = Modeller(protein.topology, protein.positions)

system = ff.createSystem(modeller.topology, nonbondedMethod = NoCutoff, constraints = HBonds)
integrator = LangevinMiddleIntegrator(310.15 * kelvin, 1/picosecond, 0.004 * picosecond)
...
simulation.reporters.append(DCDReporter('test.dcd', 100))

simulation.step(5000)

positions = simulation.context.getState(positions = True).getPositions()
PDBFile.writeFile(simulation.topology, positions, open(str(sys.argv[2]), 'w'))
