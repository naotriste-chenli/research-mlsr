...
EOF
	...
	prime-run ./QuickVina2-GPU-2-1 --receptor "${SPECIFIC_DIR}/${LIGAND}_pdockready.pdbqt" --ligand "${SPECIFIC_DIR}/${LIGAND}_ldockready.pdbqt" --search_depth $GYR_CONST --num_modes $GYR_CONST --size_x $GYRATION --size_y $GYRATION --size_z $GYRATION --center_x $X --center_y $Y --center_z $Z --thread 8000 --out ${LIGAND}_out.pdbqt
