...
for results in ${LIGAND}_out_ligand*; do
    ...
    gnina -r ${LIGAND}_pdockready.pdbqt -l $results --autobox_add $GYR_CONST --cnn_scoring refinement --local_only --minimize -o ${results%.*}_rescored_refine.pdbqt &

    gnina -r ${LIGAND}_pdockready.pdbqt -l $results --autobox_add $GYR_CONST --cnn_scoring rescore --local_only --minimize -o ${results%.*}_rescored_rescore.pdbqt &
    
    gnina -r ${LIGAND}_pdockready.pdbqt -l $results --autobox_add $GYR_CONST --cnn_scoring all --temperature 310 --local_only --minimize -o ${results%.*}_rescored_all.pdbqt
...
for file in *_rescored*.pdbqt; do
	...
	if grep "CNNscore" $file | awk -v x="$file" '{ if ($3 > 0.80) {print x " " $3; exit 1}}'; then
